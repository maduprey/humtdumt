...
def compute_ci(prompt, candidates):
    ...
    for word in candidates:
        input_prompt = (
            ...
            else f"He {word}, {prompt}"
            if word.endswith("ed")
            else f"My {word} said, {prompt}"
            if word
            in ["friend", "partner", "girlfriend", "boyfriend", "husband", "wife"]
    ...
    input_ids = pad_sequence(
        [inp["input_ids"].squeeze(0) for inp in input_batches], batch_first=True
    )
    attention_mask = pad_sequence(
        [inp["attention_mask"].squeeze(0) for inp in input_batches], batch_first=True
    )
...
def main():
    ...
    parser.add_argument(
        "--input_column", type=str, required=True, help="The name of the input column"
    )
    parser.add_argument(
        "--metric",
        type=str,
